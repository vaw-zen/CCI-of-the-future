...
const NavLink = memo(function NavLink({ link, name, isActive }) {
    return (
        <Link
            href={link}
...
const DropdownButton = memo(function DropdownButton({ name, onClick, isActive, hasActiveSublink }) {
    return (
        <button
            onClick={onClick}
...
const DropdownMenu = memo(function DropdownMenu({ isActive, subLinks, isLinkActive }) {
    return (
...
const NavItem = memo(function NavItem({ element, index, handleDropdownBlur, toggleDropdown, isActive, isLinkActive, hasActiveSublink }) {
    return (
        <li
            key={index}
    ...
    return (
        <>
            ...
            <div className={styles.stickyBottom}>
                <AnalyticsLink href="https://wa.me/21698557766?text=Bonjour%2C%20je%20souhaite%20un%20devis%20gratuit.%20Merci%20!" target="_blank" rel="noopener noreferrer" className={styles.whatsappButton} aria-label="Contacter via WhatsApp" eventLabel="sticky_header_whatsapp">
